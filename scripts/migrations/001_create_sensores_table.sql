-- Migration: Create sensores table with TimescaleDB
-- Run this migration after setting up PostgreSQL + TimescaleDB
-- Usage: psql -U TIWater_user -d TIWater_timeseries -f scripts/migrations/001_create_sensores_table.sql
...
COMMENT ON COLUMN sensores.timestamp IS 'Timestamp when the sensor reading was taken (used for TimescaleDB partitioning)';
...
-- GRANT ALL PRIVILEGES ON TABLE sensores TO TIWater_user;
-- GRANT USAGE, SELECT ON SEQUENCE sensores_id_seq TO TIWater_user;

-- Display success message
DO $$
BEGIN
    RAISE NOTICE '✅ sensores table created successfully with TimescaleDB hypertable';
